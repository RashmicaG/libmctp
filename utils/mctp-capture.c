...
void capture_socket(pcap_dumper_t *dumper, const void *buf, size_t len, bool outgoing)
{
	struct pcap_pkthdr hdr;
	int rc;
	uint8_t *pktbuf = NULL;
	...
	/* Write sll2 header */
	size = sizeof(struct sll2_header) + len;
	pktbuf = __mctp_alloc(size);
	if (!pktbuf)
		return;
	...
	memcpy(pktbuf + sizeof(struct sll2_header), buf, len);
